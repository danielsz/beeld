...
- Scaling an image in portrait mode or landscape mode based on its metadata. 
- Create a naming scheme for a photo library where parts of the name
  come from data extracted from the metadata.
...
and returns a data structure representing the metadata associated with
